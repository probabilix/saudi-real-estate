...
import {
  ...
  ChevronLeft, ChevronRight, FileText, Menu
} from 'lucide-react';
...
<div style="width:${PAGE_W}px;box-sizing:border-box;padding:${PAD}px;background:#fff;direction:${isAr ? 'rtl' : 'ltr'};">
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
      <CrmTopBar
        title="My Real Estate Projects"
        subtitle="Developer Workspace & Project Listings"
      />
      ...
      <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto w-full">
        ...
        {/* Header Bar */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-black text-white mt-1">My Real Estate Projects</h1>
              <p className="text-slate-400 text-xs sm:text-sm mt-0.5">Manage your compound and residential development bulk listings</p>
            </div>
          </div>

          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <Link href="/billing" className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl text-xs font-bold transition-all">
              <CreditCard className="w-4 h-4 text-primary-400" />
              <span>Balance: <strong className="text-white font-extrabold">{user?.creditsBalance ?? 0}</strong> Credits</span>
            </Link>
            <Link href="/my-projects/create" className="flex items-center gap-2 bg-primary-600 hover:bg-primary-500 text-white font-bold px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl text-xs sm:text-sm shadow-md transition-all">
      ...
      {loading ? (
        ...
        <>
          {/* Mobile View: Responsive Cards (prevents table widening stretch on small screens) */}
          <div className="md:hidden space-y-4">
            {filteredProjects.map(project => (
              <div key={project.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm space-y-4">
                {/* Header Info */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="font-bold text-slate-900 text-sm truncate">{project.nameEn}</span>
                        {project.isFeatured && (
                          <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-500 shrink-0" />
                        )}
                      </div>
                      <div className="text-[10px] text-slate-400 font-arabic truncate" dir="rtl">{project.nameAr}</div>
                      <div className="flex items-center gap-1.5 mt-1 text-xs text-slate-500">
                        <MapPin className="w-3 h-3 text-slate-400 shrink-0" />
                        <span className="truncate">{project.district ? `${project.district}, ` : ''}{project.city}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className={clsx(
                      'inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-all border shrink-0',
                ...
                <div className="flex items-center gap-1.5 flex-wrap pt-2 border-t border-slate-100">
                  ...
                  {project.status === 'DRAFT' && (
                    <span className="inline-flex items-center gap-1 bg-slate-100 text-slate-700 border border-slate-200 px-2 py-0.5 rounded-full text-[10px] font-bold">
                      <EyeOff className="w-3 h-3 text-slate-500" /> Draft / Hidden
                    </span>
                  )}
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-0.5 text-[9px] font-bold text-primary-700 bg-primary-50 px-2 py-0.5 rounded-full border border-primary-100"
                  >
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-slate-100 flex-wrap">
                  {project.status === 'ACTIVE' && (
                    <button
                      ...
                      className={clsx(
                        "inline-flex items-center gap-1 font-bold px-2.5 py-1.5 rounded-lg text-[10px] transition-all border",
                        project.isFeatured 
                  ...
                  <button
                    ...
                    className="p-1.5 text-slate-500 border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50"
                    title="Delete Project"
                  >
                    {deletingId === project.id ? <Loader2 className="w-4 h-4 animate-spin text-red-600" /> : <Trash2 className="w-4 h-4" />}
                  </button>
                </div>

                {/* Expanded Floor Plans on Mobile */}
                {expandedProjectId === project.id && (
                  <div className="pt-3 border-t border-slate-200 space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="text-[10px] font-black uppercase text-slate-500">Floor Plans</h4>
                      <button
                        onClick={() => {
                        ...
                        className="px-2.5 py-1 text-[10px] font-bold bg-slate-100 border border-slate-200 rounded-lg flex items-center gap-1"
                      >
                        {exportingProjectId === project.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <FileText className="w-3 h-3" />}
                        PDF Matrix
                      </button>
                    </div>
                    ...
                    {layoutsLoading[project.id] ? (
                      <div className="flex items-center gap-2 text-xs text-slate-500 py-2">
                        <Loader2 className="w-3.5 h-3.5 animate-spin text-primary-600" /> Loading layouts...
                      </div>
                    ) : !projectLayouts[project.id] || projectLayouts[project.id].length === 0 ? (
                      <p className="text-xs text-slate-400 italic">No layouts found.</p>
                    ) : (
                      <div className="grid grid-cols-1 gap-3">
                        {projectLayouts[project.id].map(layout => (
                          <div key={layout.id} className="bg-slate-50 rounded-xl p-3 border border-slate-200 flex items-center justify-between gap-3">
                            <div className="min-w-0">
                              <div className="text-xs font-bold text-slate-900 truncate">{layout.enTitle || layout.arTitle}</div>
                              <div className="text-xs font-black text-primary-600 mt-0.5">SAR {layout.price?.toLocaleString()}</div>
                              <div className="text-[10px] text-slate-500 mt-0.5">
                                {layout.areaSqm ? `${Number(layout.areaSqm).toFixed(0)} sqm` : ''} {layout.bedrooms !== null ? `• ${layout.bedrooms} BR` : ''}
                              </div>
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0">
                              <button
                                onClick={() => handleManageInventory(layout, project)}
                                className="px-2.5 py-1 bg-primary-600 text-white rounded-lg text-[10px] font-bold shadow-sm flex items-center gap-1"
                              >
                                <Building2 className="w-3 h-3" /> Inventory
                              </button>
                            </div>
                          </div>
                        ))}
