...
export default function FavoritesPage() {
  ...
  async function fetchProjectFavorites() {
    setProjectsLoading(true);
    try {
      const res = await api.getProjectFavorites();
      if (res.success && res.data) {
        setSavedProjects(res.data.items);
      }
    } catch (err) {
      console.error('Failed to fetch project favorites:', err);
    } finally {
  ...
  const filteredProjects = savedProjects.filter(p => {
    const title = locale === 'ar' ? p.nameAr : (p.nameEn || p.nameAr);
    const city = locale === 'ar' ? p.arCity || p.city : p.city;
    const district = locale === 'ar' ? p.arDistrict || p.district : p.district;
    const locationStr = [district, city].filter(Boolean).join(' ');
    return title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      locationStr.toLowerCase().includes(searchQuery.toLowerCase());
  });

  const totalFavoritesCount = activeTab === 'properties' ? listings.length : savedProjects.length;
  ...
  return (
    <div className="min-h-screen bg-surface-50/50 pb-10">
      ...
      {/* ── Tabs (Properties & Projects) ── */}
      <div className="bg-white border-b border-surface-150">
        <div className="max-w-7xl mx-auto px-6 flex gap-8">
          <button
            onClick={() => setActiveTab('properties')}
            className={`pb-4 pt-2 text-sm font-black uppercase tracking-wider border-b-2 transition-all relative ${activeTab === 'properties' ? 'border-primary-600 text-primary-600' : 'border-transparent text-charcoal-muted hover:text-charcoal'}`}
          >
            {isRTL ? 'العقارات' : 'Properties'}
            <span className={`ms-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold ${activeTab === 'properties' ? 'bg-primary-50 text-primary-600' : 'bg-surface-100 text-charcoal-muted'}`}>
              {listings.length}
            </span>
          </button>
          <button
            onClick={() => setActiveTab('projects')}
            className={`pb-4 pt-2 text-sm font-black uppercase tracking-wider border-b-2 transition-all relative ${activeTab === 'projects' ? 'border-primary-600 text-primary-600' : 'border-transparent text-charcoal-muted hover:text-charcoal'}`}
          >
            {isRTL ? 'المشاريع' : 'Projects'}
            <span className={`ms-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold ${activeTab === 'projects' ? 'bg-primary-50 text-primary-600' : 'bg-surface-100 text-charcoal-muted'}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-6">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            placeholder={
              activeTab === 'properties'
                ? t('listingsPage.searchPlaceholder', { type: t('menu.favorites') })
                : (isRTL ? 'ابحث في المشاريع المفضلة...' : 'Search in favorite projects...')
            }
        ...
        {/* ── Loading Spinner for Project Tab transition ── */}
        {((activeTab === 'properties' && loading) || (activeTab === 'projects' && projectsLoading)) ? (
          ...
          filteredProjects.length > 0 ? (
              ...
              <AnimatePresence mode="popLayout">
                {filteredProjects.map((item, i) => (
                  ...
                  >
                    <ProjectCard
                      project={item}
                      layoutCount={item.layoutCount}
                      minPrice={item.minPrice}
                      minBedrooms={item.minBedrooms}
                      maxBedrooms={item.maxBedrooms}
                      isFavorited={true}
                      index={i}
                      onToggleFavorite={(id, favorited) => {
                        if (!favorited) {
            ...
            >
              ...
              <h3 className="text-2xl font-black text-charcoal mb-3">
                {searchQuery ? 'No matching favorites' : (isRTL ? 'لا توجد مشاريع مفضلة بعد' : 'No Favorite Projects')}
              </h3>
              <p className="text-charcoal-muted max-w-sm mb-10 font-medium leading-relaxed">
                {searchQuery ? 'Try a different search term.' : 'Project developments you save will appear here for quick access.'}
              </p>
              {!searchQuery && (
                <Link
