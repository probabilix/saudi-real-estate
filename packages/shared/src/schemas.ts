...
export const registerSchema = z.object({
  ...
  phone: z.preprocess((val) => (val === '' ? undefined : val), z.string().regex(/^\+[1-9]\d{1,14}$/).optional().nullable()),
...
export const createListingSchema = z.object({
  ...
  arCity: z.string().max(100).optional(),
  district: z.string().max(100).optional(),
  arDistrict: z.string().max(100).optional(),
  ...
  livingRooms: z.number().int().min(0).max(50).optional(),
  ...
  history: z.array(z.object({
    year: z.number(),
    event: z.enum(['SOLD', 'LISTED', 'PRICE_DROP', 'RENTED']).optional(),
    price: z.number(),
