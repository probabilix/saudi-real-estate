...
export default function CalculatorLeadsPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const res = await adminApi.getCalculatorLeads();
  ...
  const handleUpdateStatus = async (userId: string, status: string) => {
    ...
    try {
      const res = await adminApi.updateCalculatorLeadStatus(userId, status);
      if (res.success) {
  ...
  const handleSaveNotes = async () => {
    ...
    setSavingNotes(true);
    try {
      const res = await adminApi.updateCalculatorLeadStatus(selectedLead.userId, undefined, notesText.trim());
      if (res.success) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-surface-50 overflow-y-auto">
      <AdminTopBar title="Calculator Leads" />
