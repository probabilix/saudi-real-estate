...
export default function CreateDeveloperProjectPage() {
  const router = useRouter();
  const { user } = useCrmAuth();
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [projectCost, setProjectCost] = useState(50);
  ...
  const [completionStatus, setCompletionStatus] = useState<'READY' | 'OFF_PLAN' | 'UNDER_CONSTRUCTION'>('OFF_PLAN');
  ...
  const [layouts, setLayouts] = useState<LayoutRow[]>([
    { labelEn: 'Type A', labelAr: 'نموذج أ', price: '', areaSqm: '', bedrooms: '3', bathrooms: '3', photos: [], completionStatus: '', descriptionEn: '', descriptionAr: '' }
  ]);
  ...
  async function fetchProjectCost() {
    try {
      const apiBase = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1';
      const res = await fetch(`${apiBase}/system/settings`);
      const json = await res.json();
      if (json.success && json.data?.project_cost_credits) {
        setProjectCost(parseInt(json.data.project_cost_credits, 10) || 50);
      }
    } catch (e) {
      console.error('Failed to fetch project cost setting:', e);
    }
  }

  // Local draft management
  useEffect(() => {
    const saved = localStorage.getItem('tamleeq_developer_project_draft_v2');
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (nameEn || nameAr || district || descriptionEn || layouts.length > 1 || layouts[0].price || photos.length > 0) {
      const draft = {
        ...
        totalUnits, mapEmbedUrl, photos, amenities, foreignerEligible, muslimOnly,
        layouts,
        updatedAt: new Date().toISOString()
      };
      localStorage.setItem('tamleeq_developer_project_draft_v2', JSON.stringify(draft));
    }
  }, [nameEn, nameAr, city, district, descriptionEn, descriptionAr, brochureUrl, brochureUrlAr, regaFalLicense, completionStatus, expectedDelivery, totalUnits, mapEmbedUrl, photos, amenities, foreignerEligible, muslimOnly, layouts]);
  ...
  const restoreDraft = () => {
    ...
    if (saved) {
      try {
        ...
        setCompletionStatus(parsed.completionStatus || 'OFF_PLAN');
  ...
  const discardDraft = () => {
    localStorage.removeItem('tamleeq_developer_project_draft_v2');
    setHasDraft(false);
  };
  ...
  const hasEnoughCredits = userBalance >= projectCost;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasEnoughCredits) {
      setError(`Insufficient credits. You need ${projectCost} credits to publish a project.`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/listings/projects/bulk`, {
        method: 'POST',
        headers: {
      ...
      if (json.success) {
        localStorage.removeItem('tamleeq_developer_project_draft_v2');
        router.push('/my-projects?success=submitted');
      } else {
        setError(json.message || 'Failed to submit project listing.');
      }
    } catch (err: any) {
    ...
    <div className="flex flex-col h-full bg-slate-50">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-primary-500/20 text-primary-400 border border-primary-500/30 text-[10px] font-bold uppercase tracking-wider">
                  Developer Project Wizard
                </span>
              </div>
              <h1 className="text-xl font-black text-white mt-1">List New Real Estate Project</h1>
              <p className="text-xs text-slate-400">Publish a compound or residential housing project with multi-unit layout models</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <Link href="/billing" className="flex items-center gap-2 bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-200 px-3.5 py-2 rounded-xl text-xs font-bold transition-all">
              <CreditCard className="w-4 h-4 text-primary-400" />
              <span>Balance: <strong className="text-white font-extrabold">{userBalance}</strong> Credits</span>
            </Link>
            <div className="flex items-center gap-1.5 bg-amber-500/20 text-amber-300 border border-amber-500/30 px-3.5 py-2 rounded-xl text-xs font-bold">
              <Sparkles className="w-4 h-4 text-amber-400 shrink-0" />
              <span>Listing Cost: {projectCost} Credits</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          {!hasEnoughCredits && (
            <div className="p-4 bg-red-50 border border-red-200 rounded-2xl flex items-center justify-between gap-3 text-red-800 text-xs font-bold shadow-sm">
              <div className="flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-red-600 shrink-0" />
                <span>You have {userBalance} credits. You need {projectCost} credits to publish a project listing.</span>
              </div>
              <Link href="/billing" className="bg-red-600 hover:bg-red-700 text-white px-3.5 py-1.5 rounded-xl text-xs transition-all shadow-sm">
                Purchase Credits
              </Link>
            </div>
          )}
          ...
          {error && (
            <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-700 text-sm font-bold shadow-sm">
          ...
          <form id="developer-project-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    dir="rtl"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-arabic focus:ring-2 focus:ring-primary-500 outline-none animate-none"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setRegaFalLicense(e.target.value)}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <textarea
                    rows={4}
                    dir="rtl"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-arabic focus:ring-2 focus:ring-primary-500 outline-none resize-none"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    setMapEmbedUrl(match ? match[1] : val);
                  }}
                />
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <div className="text-xs text-slate-400 font-semibold">
                  Selected: <strong className="text-primary-600">{Object.values(amenities).filter(Boolean).length}</strong>
                </div>
              </div>
                ...
                {AMENITY_OPTIONS.map(a => {
                  ...
                  return (
                    <label key={a.key} className={clsx(
                      ...
                      <div className={clsx('w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors', checked ? 'bg-primary-600 border-primary-600 text-white' : 'border-slate-300 bg-white')}>
              ...
              {dynamicAmenities.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-2">
                  {dynamicAmenities.map(id => (
                    <span key={id} className="inline-flex items-center gap-1 bg-primary-105 text-primary-800 border border-primary-200 px-2.5 py-1 rounded-lg text-xs font-bold">
                      {id.replace(/_/g, ' ')}
      ...
      <div className="bg-white border-t border-slate-200 py-3 px-6 flex items-center justify-between shrink-0 shadow-[0_-4px_16px_rgba(0,0,0,0.06)] z-20">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {saving ? (
              <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Submitting Project...</>
            ) : (
              <><Save className="w-3.5 h-3.5" /> Submit &amp; Deduct {projectCost} Credits</>
            )}
          </button>
        </div>
      </div>
